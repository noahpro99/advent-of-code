...
fn apply_rule_w_maxes(part: &Part, rule: Rule) -> (Option<RuleResult>, (i64, i64, i64, i64)) {
    let part_category = match rule.category {
        Category::X => part.x,
        Category::M => part.m,
        Category::A => part.a,
    ...
    match rule.comparison {
        Comparison::LessThan => {
            if part_category < rule.value && rule.value < part_category {
                (Some(rule.result), maxes)
            } else {
                (None, maxes)
            }
        }
...
fn apply_workflow_w_maxes(part: &Part, workflow: &Workflow) -> (RuleResult, (i64, i64, i64, i64)) {
    let mut result = workflow.else_result.clone();
    let mut maxes = (4001, 4001, 4001, 4001);
    for rule in &workflow.rules {
        let (rule_result, rule_maxes) = apply_rule_w_maxes(part, rule.clone());
        maxes.0 = std::cmp::min(maxes.0, rule_maxes.0);
        maxes.1 = std::cmp::min(maxes.1, rule_maxes.1);
        maxes.2 = std::cmp::min(maxes.2, rule_maxes.2);
        maxes.3 = std::cmp::min(maxes.3, rule_maxes.3);
...
fn accept_or_reject_w_maxes(
    part: &Part,
    workflows: &HashMap<&str, Result<Workflow, String>>,
) -> (FinalResult, (i64, i64, i64, i64)) {
    let mut curr = workflows.get("in").unwrap().as_ref().unwrap();
    let (mut max_x, mut max_m, mut max_a, mut max_s) = (4001, 4001, 4001, 4001);
    loop {
        let (result, workflow_maxes) = apply_workflow_w_maxes(part, curr);
        max_x = std::cmp::min(max_x, workflow_maxes.0);
        max_m = std::cmp::min(max_m, workflow_maxes.1);
        max_a = std::cmp::min(max_a, workflow_maxes.2);
        max_s = std::cmp::min(max_s, workflow_maxes.3);
        match result {
            RuleResult::Accept => return (FinalResult::Accept, (max_x, max_m, max_a, max_s)),
            RuleResult::Reject => return (FinalResult::Reject, (max_x, max_m, max_a, max_s)),
            RuleResult::Next(next) => {
                curr = workflows.get(next.as_str()).unwrap().as_ref().unwrap();
            }
        }
